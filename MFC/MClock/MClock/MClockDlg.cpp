...
BOOL CMClockDlg::OnInitDialog()
{
	...
	screen.right = ::GetSystemMetrics(SM_CYSCREEN);

	GetWindowRect(rect);
	vsize = rect.Width();
	hsize = rect.Height();

	htimer = SetTimer(1, 100, NULL);
...
void CMClockDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	GetDlgItem(IDC_STATIC_WEEK)->SetWindowText((LPCTSTR)strWeek);
	...
	if (gct.GetHour() < 12 || gct.GetHour() == 24)
	{
		strMa = _T("오전");
	}
	else
